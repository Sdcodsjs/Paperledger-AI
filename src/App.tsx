    ...
    for (const file of acceptedFiles) {
      try {
        ...
        if (isExcel) {
          ...
          extracted = await extractFromText(textContent, (done, total) => {
        ...
        // Ensure extracted is an array
        const extractedItems = Array.isArray(extracted) ? extracted : [extracted];

        if (extractedItems.length === 0 || typeof extractedItems[0].totalAmount !== 'number') {
           throw new Error("Failed to extract valid data from document.");
        }

        const newDocs: Document[] = extractedItems.map(ex => ({
          id: Math.random().toString(36).substring(2, 11),
          vendor: ex.vendor || 'Unknown Vendor',
          ...
          isFraudulent: ex.isFraudulent || false,
          fraudReason: ex.fraudReason,
          status: ex.isFraudulent ? 'flagged' : 'processed',
          carbonFootprintKg: ex.carbonFootprintKg || 0,
          taxDeductibleScore: ex.taxDeductibleScore || 0,
          taxOptimizationTip: ex.taxOptimizationTip,
          isSubscription: ex.isSubscription || false,
          subscriptionFrequency: ex.subscriptionFrequency,
          confidenceScore: ex.confidenceScore || 0,
          vendorReliabilityScore: ex.vendorReliabilityScore || 0,
          smartTags: ex.smartTags || [],
        ...
        <div className="p-8 max-w-7xl mx-auto">
          ...
          {chunkProgress && (
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              className="mb-8 p-4 bg-emerald-500/10 border border-emerald-500/50 rounded-2xl"
            >
